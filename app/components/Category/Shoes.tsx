    ...
    useEffect(() => {
        const fetchMaterials = async () => {
            try {
                ...
                // Filter for Shoes category
                const shoes = data.filter((item: Material) =>
                    item.category?.name?.toLowerCase() === 'shoes'
                );

                setMaterials(shoes);
                setFilteredMaterials(shoes);
            } catch (error) {
            ...
            <div className="relative bg-[#1A1A1A] text-white py-20 px-4 mb-12">
                <div className="max-w-7xl mx-auto space-y-6">
                    ...
                    <p className="text-gray-400 max-w-2xl text-lg font-light">
                        Step with confidence. Our curated selection of premium footwear blends
                        style, comfort, and durability for every occasion.
                    </p>
                </div>
